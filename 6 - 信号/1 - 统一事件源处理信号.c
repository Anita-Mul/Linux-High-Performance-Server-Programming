 ...
 * 那么主循环怎么知道管道上何时有数据可读呢?这很简单，我们只需要使用I/O
 * 复用系统调用来监听管道的读端文件描述符上的可读事件。如此一来，信号事
 * 件就能和其他I/O事件一样被处理，即统一事件源
 */
...
void addsig( int sig )
{
    ...
    sigfillset( &sa.sa_mask );   // 全部置1
    assert( sigaction( sig, &sa, NULL ) != -1 );  // 注册信号捕捉函数
}
