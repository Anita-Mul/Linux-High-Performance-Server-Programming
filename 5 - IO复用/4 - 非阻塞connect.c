 ...
 * 
 * 在这种情况下，我们可以调用select、poll等函数来监听这个连接失败的socket上的可写事件。当select、poll等函数返
 * 回后，再利用getsockopt来读取错误码并清除该socket上的错误。如果错误码是0，表示连接成功建立，否则连接失败
 * 
...
int unblock_connect( const char* ip, int port, int time )
{
    ...
    int fdopt = setnonblocking( sockfd );   // 旧的文件描述符属性
