...
{
    sockaddr_in address;
    char* write_buf;    // 将来要写道这个 socket 中的数据
    char buf[ BUFFER_SIZE ];   // 从客户端读到的数据
};
    ...
    while( 1 )
    {
        ...
        for( int i = 0; i < user_counter+1; ++i )
        {
            if( ( fds[i].fd == listenfd ) && ( fds[i].revents & POLLIN ) )
            {
                ...
                fds[user_counter].events = POLLIN | POLLRDHUP | POLLERR;    // POLLRDHUP是对端关闭
            ...
            else if( fds[i].revents & POLLERR )
            {
                ...
                if( getsockopt( fds[i].fd, SOL_SOCKET, SO_ERROR, &errors, &length ) < 0 )  // 获取到错误
                {
            ...
            else if( fds[i].revents & POLLIN )
            {
                ...
                else
                {
                    ...
                    for( int j = 1; j <= user_counter; ++j )
                    {
                        ...
                        fds[j].events |= ~POLLIN; // /* There is data to read */
                        fds[j].events |= POLLOUT; // /* Writing now will not block */
